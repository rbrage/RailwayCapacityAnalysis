...
global_info.next_train_time_Stavanger = [[05 00];[05 16];[05 31]];
%global_info.next_train_time_Sandnes = [[05 00];[05 16];[05 31]];
global_info.next_train_type = [double('N') double('S')];
initokensStavanger = length(global_info.next_train_time_Stavanger);
%initokensSandnes = length(global_info.next_train_time_Sandnes);

pns = pnstruct('RailwaySim_pdf');

dyn.m0 = {'pGenStavanger', initokensStavanger};%, 'pGenSandnes', initokensSandnes};%How many tokens that are in places
